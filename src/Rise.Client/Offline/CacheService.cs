...
public class CacheService(IJSRuntime jsRuntime)
{
    ...
    public async Task CacheAsync<T>(CacheObject<T> objectToCache, CancellationToken cancellationToken)
    {
        try
        {
            var payload = JsonSerializer.Serialize(objectToCache, _serializerOptions);
            await _jsRuntime.InvokeVoidAsync("localStorage.setItem", cancellationToken, objectToCache.Key, payload);
        }
        catch
        {
